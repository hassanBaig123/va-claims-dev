...
import { ConditionsList } from './condition-search-box/ConditionsList'
...
export function ConditionSearchBox({
  ...
  const handleSelect = (conditionValue: string) => {
    if (conditionValue === 'no_conditions') {
      ...
      setConditionsList([])
      onChange && onChange([])
      setSelectedCondition({ value: 'no_conditions', label: 'No conditions', code: '', category: '' })
      setOpen(false)
      return
    }
          ...
          <DialogHeader>
            {
              isEditModal ? (
                <>
                  <DialogTitle className="text-center mb-2.5">Edit condition</DialogTitle>
                  <DialogTitle>{editingCondition?.label}</DialogTitle>
                </>
              ) :
                (
                  <>
                    <DialogTitle className="text-center mb-2.5">Add new condition</DialogTitle>
                    <Popover open={open} onOpenChange={setOpen}>
                      <div className="grid grid-cols-2 items-center gap-4 w-1/2">
                        ...
                        <PopoverTrigger asChild>
                          ...
                          >
                            {selectedCondition ? selectedCondition.label : (placeholder || 'Select condition...')}
                            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                          </Button>
                        </PopoverTrigger>
                      </div>
                      <PopoverContent className="w-[200px] max-h-72 overflow-y-auto p-0"
                          ...
                          <CommandGroup>
                            ...
                            >
                              <Check
                                className={cn(
                                  'mr-2 h-4 w-4',
                                  conditionsList.length === 0 ? 'opacity-100' : 'opacity-0',
                                )}
                              />
                              No conditions
                            </CommandItem>
                            {conditions.map((condition: Condition, index: number) => (
                              <CommandItem
                                key={`${condition.code + index + condition.value}`}
                              ...
                              >
                                <Check
                                  className={cn(
                                    'mr-2 h-4 w-4',
                                    conditionsList.some((c) => c.value === condition.value)
                                      ? 'opacity-100'
                                      : 'opacity-0',
                                  )}
                                />
            ...
            <DialogDescription>
              {
                (isEditModal ||
                  (selectedCondition && selectedCondition.value !== 'no_conditions')) &&
