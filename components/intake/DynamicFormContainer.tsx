...
import { ProcessState, QuestionnaireData } from '@/models/local/types';
...
const DynamicFormContainer: React.FC<DynamicFormContainerProps> = (
    ...
    const handleSocketErrorMessage = useCallback((sessionId: string, eventData: any) => {
        console.error('Error:', eventData.error);
    }, []);

    const handleSocketSessionMessage = useCallback((sessionId: string, eventData: any) => {
        console.log('Session:', eventData);
    }, []);

    const handleSocketUnknownMessage = useCallback((sessionId: string, eventData: any) => {
        try {
    ...
    const handleSocketTaskMessage = useCallback((_: string, eventData: TaskCallback) => {
    ...
    }, [answers, setQuestionnaireData]);

    const handleProcessStateChange = useCallback((sessionId: string, newState: ProcessState, message?: any) => {
    ...
    const handleSocketStatusMessage = useCallback(async (eventData: StatusUpdate) => {
        if(!user) return;
    ...
    }, [user, authenticateSession, subscribeToSession, handleProcessStateChange, setSessionId]);

    const handleSocketStatusMessageMemo = useCallback(handleSocketStatusMessage, [handleSocketStatusMessage]);
    const handleSocketTaskMessageMemo = useCallback(handleSocketTaskMessage, [handleSocketTaskMessage]);
        ...
        const immediateUpdateHandler = async (update: Update) => {
            switch (update.type) {
                case 'status':
                    ...
                    await handleSocketStatusMessageMemo(update);
                    break;
                case 'error':
                ...
                case 'session':
                    ...
                    break;
                case 'task':
                    handleSocketTaskMessageMemo(update.sessionId, update);
                    break;
                default:
    ...
    }, [user, setUpdateHandler, handleSocketStatusMessageMemo, handleSocketTaskMessageMemo, handleSocketErrorMessage, handleSocketSessionMessage, handleSocketUnknownMessage]);
    ...
    const isGroupComplete = useCallback((groupIndex: number, answers: AnswersState[], questionnaireData: QuestionnaireData) => {
    ...
    }, [currentGroupIndex, answers, onGroupCompletionChange, isGroupComplete, questionnaireData]);
