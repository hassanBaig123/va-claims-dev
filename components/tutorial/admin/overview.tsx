...
const aggregateDataByQuarter = (data: any[]) => {
  const quarters = [
    { name: 'Q1', months: ['Jan', 'Feb', 'Mar'], total: 0 },
    { name: 'Q2', months: ['Apr', 'May', 'Jun'], total: 0 },
    { name: 'Q3', months: ['Jul', 'Aug', 'Sep'], total: 0 },
    { name: 'Q4', months: ['Oct', 'Nov', 'Dec'], total: 0 },
  ];

  data.forEach((item: { name: string; total: number; }) => {
    const quarter = quarters.find(q => q.months.includes(item.name));
    if (quarter) {
      quarter.total += item.total;
    }
  });
...
const quarterlyData = aggregateDataByQuarter(data);
...
export function Last12MonthsSales() {
  ...
  return (
    ...
    <ResponsiveContainer width="100%" height={350}>
      <BarChart data={isQuarterly ? quarterlyData : data}>
