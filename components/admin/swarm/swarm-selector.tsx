...
export function SwarmSelector({
  ...
  const defaultSwarm = useMemo(() => {
    return (
      swarms.find((swarm) =>
        selectedSwarmId ? swarm.id === selectedSwarmId : swarm.default,
      ) || swarms[0]
    )
  }, [swarms, selectedSwarmId])

  const [selectedSwarm, setSelectedSwarm] = React.useState<Swarm | undefined>(
    defaultSwarm,
  )
  ...
  useEffect(() => {
    if (selectedSwarmId) {
      ...
      console.log(
        'No selectedSwarmId provided, using default swarm: ',
        defaultSwarm,
      )
  ...
  }, [selectedSwarmId, swarms, defaultSwarm])
  ...
  return (
    <Popover open={open} onOpenChange={setOpen} {...props}>
      ...
      <PopoverContent className="w-[300px] p-0">
        <Command>
          <Input placeholder="Search swarms..." />
