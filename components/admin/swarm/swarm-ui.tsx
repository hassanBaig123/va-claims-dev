...
export function SwarmUI({ user }: SwarmUIProps) {
  ...
  const handleProcessStateChange = (newState: ProcessState) => {
    ...
    toast({
      ...
      description: <pre>{`Process state changed to ${newState}`}</pre>,
    })
  }

  const updateSwarmSessions = async (
    user_id: string,
  ...
  const handleAddSession = async () => {
    ...
    if (!sessions) {
      updatedSessions = [
        { id: tempSessionId, name: `Session 1`, description: '' },
      ]
    } else {
      updatedSessions = sessions?.concat({
        id: tempSessionId,
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(
      `ws://${process.env.NEXT_PUBLIC_WEB_SOCKET}/ws/agency_swarm`
    )
    socket.onopen = () => {
        ...
        if (result.status) {
          switch (result.status) {
            ...
            case 'connected':
              ...
              handleProcessStateChange('CONNECTED')
              socket.send(
                JSON.stringify({ action: 'new_session', sessionId: sessionId })
              )
              break
          ...
          <div className="flex flex-grow flex-col bg-white h-full justify-end">
            ...
            <div className="flex bg-gray-100 rounded p-2">
              <Input
                ...
                onKeyUp={(e) => e.key === 'Enter' && handleAddMessage()}
              />
              <Button
                id="send-message"
                type="submit"
