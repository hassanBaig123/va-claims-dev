...
export async function getFinancialMetrics() {
  ...
  try {
    ...
    const totalRevenue = payments.data.reduce((acc, payment) => acc + payment.amount_received, 0);
    const subscriptionCount = payments.data.filter(payment => payment.metadata.subscription === 'true').length;
...
async function fetchAndAggregateSalesData(period: 'monthly' | 'quarterly'): Promise<SalesDataItem[]> {
  const payments = await stripe.paymentIntents.list({
    limit: 100,
  });
