...
export async function changeFormStatus({ form_id, status }: { form_id: string, status: FormsStatus }): Promise<{ data: any, error: any }> {
    ...
    if(updated_form) {
        if(status == "submission_approved") {
            ...
            if(!unapprovedFormsData && !formsError) {
                ...
                const { data: scheduledEventData, error: scheduledEventError } = await supabase
