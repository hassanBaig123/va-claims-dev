...
import type { Database, Tables, TablesInsert } from '@/models/supabase/supabase'
...
const upsertPriceRecord = async (
  ...
  const priceData: Price = {
    ...
    description: price.nickname ?? null, // Add description property
    type: price.type,
    unit_amount: price.unit_amount ?? null,
    interval: price.recurring?.interval ?? null,
    interval_count: price.recurring?.interval_count ?? null,
    trial_period_days: price.recurring?.trial_period_days ?? TRIAL_PERIOD_DAYS,
    metadata: price.metadata, // Add metadata property
  }

  const { error: upsertError } = await client.from('prices').upsert([priceData])
