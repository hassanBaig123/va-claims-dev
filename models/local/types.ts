import { Database as DB, Json } from "@/models/supabase/supabase";

declare global {
  type Database = DB;
  type Prices = DB['public']['Tables']['prices']['Row'];
  type Customers = DB['public']['Tables']['customers']['Row'];
  type UserSubscription = DB['public']['Tables']['user_submissions']['Row'];
  ...
  type FormsStatus = DB['public']['Enums']['action'];
  type FormsType = DB['public']['Enums']['form_type'];
}



export type ProcessState = 'NOT_CONNECTED' | 'CONNECTED' | 'READY' | 'AUTHENTICATED' | 'SESSION_STARTED' | 'SUBSCRIBED' | 'JOINED' | 'LEFT' | 'DISCONNECTED' | 'ERROR' | 'NEW_CONNECTION' | 'INITIALIZING' | 'INITIALIZED' | 'RUNNING' | 'STOPPED' | 'COMPLETED';
