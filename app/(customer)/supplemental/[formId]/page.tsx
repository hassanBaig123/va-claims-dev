...
const questionnaireDataset: {pages: Page[]} = {
  ...
  const Intake2 = ({ params }: { params: { formId: string } }) => {
    ...
    useEffect(() => {
        ...
        if(params.formId) {
            console.log(`Retrieving form with formId: ${params.formId}`);
  ...
  const handleSubmit = () => {
      ...
      submitFormMutation.mutate({id: params.formId, status: "submitted", questions: mergeQuestionsAndAnswers(formState.questions, formState.answers)});
  };
