...
export async function GET(
  ...
  const { data, error } = await supabase
    ...
    .select(
      'id, title, report_type, decrypted_form, created_at, updated_at, user_id, status, type, users (id, full_name, email)',
    )
    .eq('id', params.formId)
    .single()
...
export async function PUT(req: Request) {
  ...
  const { data, error } = await supabase
    ...
    .select()
    .limit(1)
    .single() // Ensure the update is only applied to the form with the specified ID
  ...
  if (data.status == 'submission_approved') {
    ...
    switch (data.type) {
      case 'intake':
        ...
        output = {
          conditions: [{ condition: '{condition}', details: '{details}' }],
        }
        break
      case 'supplemental':
