...
export default function PlaygroundPage() {
  ...
  const [reFetchTrigger, setReFetchTrigger] = useState(0); // Add a state to trigger re-fetch
  ...
  const handleDialogSubmit = () => {
    ...
    // Function to replace placeholders with values from dialogInputs
    const replacePlaceholders = (text: string) => {
      const regex = /{{(.*?):label=(.*?),placeholder=(.*?),type=(.*?)}}/g;
      return text.replace(regex, (match: string, identifier: string, label: string) => {
        // Use the label to access the corresponding value in dialogInputs
        return dialogInputs[label] || '';
      });
    };
  
    // Replace placeholders in user_request and shared_instructions
    ...
    // Update the selectedSwarm with the modified values
    const newSwarm = {
      ...tempSwarm,
    ...
    // Continue with the rest of the session initialization logic
  ...
  // Replace the WebSocket initiation and actions with REST API calls
  ...
  const handleAddMessage = (sessionId: string) => {
    if (inputValue.trim() !== '') {
      sendMessageMutation.mutate({
        ...
        onSuccess: () => {
          setInputValue(''); // Clear the input field after successful send
        },
        onError: (error) => {
